...
const geometrySchema = Joi.object({
  ...
  coordinates: Joi.array()
    .items(Joi.array().ordered(Joi.number().required(), Joi.number().required()).length(2))
    .min(2)
    .required(),
}).required()
...
const routeSchema = Joi.object({
  ...
  type: Joi.string().valid('microbus', 'bus', 'tram', 'train', 'university_shuttle').required(),
...
const routeUpdateSchema = Joi.object({
  ...
  type: Joi.string().valid('microbus', 'bus', 'tram', 'train', 'university_shuttle').optional(),
  localName: Joi.string().optional(),
  nameAr: Joi.any().strip(),
  nameEn: Joi.any().strip(),
...
const generatePath = async (req, res, next) => {
  try {
    const result = await adminService.generateRoutePath(req.params.id, req.body.waypoints)
    res.status(200).json({ success: true, ...result })
  } catch (err) {
    next(err)
  }
...
const getRouteRatings = async (req, res, next) => {
  try {
    const result = await adminService.getRouteRatings(req.params.id)
    res.status(200).json({ success: true, ...result })
  } catch (err) {
    next(err)
  }
